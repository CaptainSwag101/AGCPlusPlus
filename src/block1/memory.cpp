...
namespace agcplusplus::block1 {
    ...
    word Memory::read(word address, word bank) {
        ...
        if (address <= MEM_ERASABLE_END) {
            ...
            if (Agc::configuration.log_memory) {
                Agc::log_stream << "Read from erasable memory: " << std::oct << std::setw(4) << address;
                Agc::log_stream << ": " << std::setw(6) << data << std::dec << std::endl;
            }
            ...
            if (address <= MEM_FIXED_FIXED_END || bank <= 3) {
                ...
                Agc::log_stream << "Invalid fixed-banked memory read: " << std::oct << std::setw(2) << bank << "," << std::setw(4) << address;
                Agc::log_stream << ": " << std::setw(6) << data << std::dec << std::endl;
            }
            ...
            data |= (data & BITMASK_16) >> 1;   // Copy bit 16 into bit 15
            if (Agc::configuration.log_memory) {
                Agc::log_stream << "Read from fixed memory: " << std::oct << std::setw(2) << bank << "," << std::setw(4) << address;
                Agc::log_stream << ": " << std::setw(6) << data << std::dec << std::endl;
            }
        } else {
            Agc::log_stream << "Invalid memory read: " << std::oct << std::setw(2) << bank << "," << std::setw(4) << address;
            Agc::log_stream << ": " << std::setw(6) << data << std::dec << std::endl;
        }
    ...
    void Memory::write(word address, word data) {
        if (address >= 020 && address <= MEM_ERASABLE_END) {
            ...
            if (Agc::configuration.log_memory) {
                Agc::log_stream << "Write to erasable memory: " << std::oct << std::setw(4) << address;
                Agc::log_stream << ": " << std::setw(6) << data << std::dec << std::endl;
            }
        }
    }

    void Memory::assign_fixed_memory(const std::vector<word>& buffer) {
